...
from src.config import get_artifact_path, load_config
...
class Predictor:
    ...
    def _load_artifacts(self):
        try:
            self.model = joblib.load(get_artifact_path("model_file"))
        ...
        logger.info("Loaded model=%s version=%s threshold=%.3f", self.model_name, self.model_version, self.threshold)
