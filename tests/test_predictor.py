...
def _fake_predictor():
    ...
    predictor.config = {"features": {"numeric_features": NUMERIC_FEATURES, "categorical_features": CATEGORICAL_FEATURES}}
    return predictor


def test_predict_one_returns_expected_keys_and_shape():
    predictor = _fake_predictor()
    features_df = pd.DataFrame([{"num_items": 2, "total_price": 100.0, "customer_state_grouped": "SP"}])
    result = predictor.predict_one(features_df)
    assert set(result.keys()) == {"is_late", "probability", "threshold_used", "model_name", "model_version"}
