...
def test_health_route_reports_model_loaded(patched_config):
...
def test_predict_route_returns_valid_prediction(patched_config):
    with TestClient(app) as client:
...
def test_predict_route_rejects_bad_payload_cleanly(patched_config):
    with TestClient(app) as client:
        response = client.post("/predict", json={**VALID_ORDER, "total_price": -1})
        assert response.status_code == 422


def test_batch_predict_route_works(patched_config):
    with TestClient(app) as client:
